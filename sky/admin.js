...
function openOpportunityDetails(title, details) {
    ...
    // show max applicants if available, else show N/A
    const applicantsText = (details.max_applicants !== undefined && details.max_applicants !== null) ? details.max_applicants : (details.applicants || 'N/A');
    document.getElementById('opportunityDetailApplicants').textContent = applicantsText;
    ...
    // Display category in the modal header (no static HTML changes)
    try {
        const modal = document.getElementById('opportunityDetailsModal');
        const header = modal.querySelector('.modal-header');
        if (header) {
            let catEl = header.querySelector('#opportunityDetailCategory');
            if (!catEl) {
                catEl = document.createElement('div');
                catEl.id = 'opportunityDetailCategory';
                catEl.style.fontSize = '13px';
                catEl.style.color = 'var(--qf-text-light)';
                catEl.style.marginTop = '6px';
                header.appendChild(catEl);
            }
            const catMap = {technology:'Technology', business:'Business', design:'Design', marketing:'Marketing', data:'Data Science', other:'Other'};
            const catLabel = details.category ? (catMap[details.category] || details.category) : '';
            catEl.textContent = catLabel ? `Category: ${catLabel}` : '';
        }
...
// (Client-side card creation removed.) The form is handled via API below to ensure
// opportunities are saved to the backend and associated with the logged-in admin.

// ===== API HELPERS & BACKEND INTEGRATION =====
...
async function loadOpportunities() {
    ...
    if (res.status === 401) {
        ...
        empty.textContent = 'No opportunities. Please sign in to view your opportunities.';
    ...
    if (!Array.isArray(data) || data.length === 0) {
        const empty = document.createElement('div');
        empty.className = 'empty-state';
        empty.textContent = 'No opportunities created yet.';
        grid.appendChild(empty);
        return;
    }
    data.forEach(op => {
        ...
        const skillsHtml = `<div class="opportunity-skills"><div class="opportunity-skills-label">Skills You'll Gain</div><div class="skills-tags">
                ${op.skills.map(s=>`<span class="skill-tag">${escapeHtml(s)}</span>`).join('')}
            </div></div>`;
        const applicantsCount = op.max_applicants ? `${parseInt(op.max_applicants,10)} applicants` : '0 applicants';
        const footerHtml = `
                <div class="opportunity-footer">
                    ...
                    <div style="display:flex;gap:8px;">
                      ...
                      <button class="edit-opp-btn" style="width: auto; padding: 8px 12px;">Edit</button>
                      <button class="delete-opp-btn" style="width: auto; padding: 8px 12px;">Delete</button>
                    </div>
                </div>
            `;
        ...
        // delete
        card.querySelector('.delete-opp-btn').addEventListener('click', async function() {
            if (!confirm('Are you sure you want to delete this opportunity?')) return;
            const resp = await fetch('/api/opportunities/' + op.id, {method:'DELETE', credentials:'include'});
            if (resp.ok) {
...
// intercept opportunity form to support create/edit via API
document.getElementById('opportunityForm').addEventListener('submit', async function(e) {
    e.preventDefault();
    // clear previous field errors
    document.querySelectorAll('#opportunityForm .input-wrap input, #opportunityForm .input-wrap textarea, #opportunityForm .input-wrap select').forEach(el => el.classList.remove('error'));

    const nameEl = document.getElementById('oppName');
    const durationEl = document.getElementById('oppDuration');
    const startDateEl = document.getElementById('oppStartDate');
    const descriptionEl = document.getElementById('oppDescription');
    const skillsEl = document.getElementById('oppSkills');
    const categoryEl = document.getElementById('oppCategory');
    const futureEl = document.getElementById('oppFuture');
    const maxApplicantsEl = document.getElementById('oppMaxApplicants');
    ...
    const futureOpportunities = futureEl.value.trim();
    const maxApplicants = maxApplicantsEl.value.trim();

    // validate required fields individually and mark errors
    const allowedCategories = ['technology','business','design','marketing','data','other'];
    const invalidFields = [];
    if (!name) { nameEl.classList.add('error'); invalidFields.push(nameEl); }
    if (!duration) { durationEl.classList.add('error'); invalidFields.push(durationEl); }
    if (!startDate) { startDateEl.classList.add('error'); invalidFields.push(startDateEl); }
    if (!description) { descriptionEl.classList.add('error'); invalidFields.push(descriptionEl); }
    if (!skillsRaw) { skillsEl.classList.add('error'); invalidFields.push(skillsEl); }
    if (!category || allowedCategories.indexOf(category) === -1) { categoryEl.classList.add('error'); invalidFields.push(categoryEl); }
    if (!futureOpportunities) { futureEl.classList.add('error'); invalidFields.push(futureEl); }
    ...
    if (skills.length === 0) {
        skillsEl.classList.add('error');
        showToast('Please provide at least one skill');
        skillsEl.focus();
        return;
    }
    ...
    if (currentEditingId) {
        const resp = await fetch('/api/opportunities/' + currentEditingId, {method:'PUT', headers:{'Content-Type':'application/json'}, body:JSON.stringify(payload), credentials:'include'});
        if (resp.ok) {
        ...
        const resp = await fetch('/api/opportunities', {method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify(payload), credentials:'include'});
        if (resp.ok) {
...
// ===== AUTH: LOGIN / SIGNUP / FORGOT (use backend) =====
document.getElementById('loginForm').addEventListener('submit', async function(e) {
    e.preventDefault(); clearAllErrors('loginForm');
    let valid = true;
    const email = document.getElementById('loginEmail').value.trim();
    const password = document.getElementById('loginPassword').value.trim();
    const captchaInput = document.getElementById('loginCaptchaInput').value.trim();
    const remember = document.querySelector('#loginForm .remember-me input')?.checked;
    ...
    const resp = await fetch('/api/login', {method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({email,password,remember}), credentials:'include'});
    if (!resp.ok) {
    ...
    setTimeout(() => showDashboard(data.email), 1200);
    generateCaptcha('login');
    await loadOpportunities();
});

document.getElementById('signupForm').addEventListener('submit', async function(e) {
    e.preventDefault(); clearAllErrors('signupForm');
    let valid = true;
    ...
    const resp = await fetch('/api/signup', {method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({name,email,password}), credentials:'include'});
    if (!resp.ok) {
        const err = await resp.json();
...
document.getElementById('forgotForm').addEventListener('submit', async function(e) {
    e.preventDefault(); clearAllErrors('forgotForm');
    let valid = true;
...
// (handlers for login/signup/forgot are implemented above with API integration)
